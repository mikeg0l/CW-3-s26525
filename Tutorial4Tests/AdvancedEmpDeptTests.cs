...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnDistinctJobTitles()
    {
        ...
        Assert.Contains("PRESIDENT",
            jobs);
        Assert.Contains("SALESMAN",
            jobs);
    }
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        Assert.Contains(result,
            r => r.Employee == "SMITH" && r.Manager == "FORD");
    }
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        {
            e.EName,
            TotalIncome = e.Sal + e.Comm.GetValueOrDefault()
        });

        Assert.Contains(result,
            r => r.EName == "ALLEN" && r.TotalIncome == 1900);
    }
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts,
            ...
            .SelectMany(ed => salgrades.Where(s => ed.e.Sal >= s.Losal && ed.e.Sal <= s.Hisal),
                (ed,
                    s) => new
                {
                    ed.e.EName,
        ...
        Assert.Contains(result,
            r => r.EName == "ALLEN" && r.DName == "SALES" && r.Grade == 3);
    }
}