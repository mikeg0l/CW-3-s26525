...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, e => e.Mgr, e => e.EmpNo, (emp, emp1) => new
        {
            Employee = emp.EName,
            Manager = emp1.EName
        }).ToList();
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        Assert.Contains(result, r => r.EName == "ALLEN" && r.TotalIncome == 1900);
    }
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new {e, d})
            .SelectMany(ed => salgrades.Where(s => ed.e.Sal >= s.Losal && ed.e.Sal <= s.Hisal), (ed, s) => new
            {
                ed.e.EName,
                ed.d.DName,
                s.Grade
